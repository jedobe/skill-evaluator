...
Be charitable to the baseline (RESPONSE_B). Many AI assistants naturally include light greetings
— score that honestly. The goal is an accurate delta, not to inflate the skill's apparent value.

Note: This is a minimal/control skill. The expected delta is near zero. A high delta would indicate
the judge is not being sufficiently charitable to the baseline.
...
- A delta_normalized close to 0 is the expected and correct result for this minimal skill.
- delta_normalized > 0.2 would indicate a surprisingly strong behavioral effect from a one-line skill.
- delta_normalized <= 0.0 means the skill made no difference or the baseline was naturally better.
